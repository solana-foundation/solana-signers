...
mod tests {
    ...
    #[tokio::test]
    #[cfg(all(feature = "integration-tests", feature = "integration-test-simulation"))]
    async fn test_turnkey_sign_transaction_with_simulation() {
        use crate::tests::rpc_util::{get_latest_blockhash, send_transaction};

        let signer = get_signer().await;

        let mut transaction = create_test_transaction(&signer.pubkey());
        transaction.message.recent_blockhash = get_latest_blockhash()
            .await
        ...
        send_transaction(&decoded_transaction)
            .await
